...
static uint32_t first_handle_from_transaction(struct binder_transaction_data *tr)
{
    ...
    printf("AOSP reply object: offset=%" PRIu64 " type=0x%08x handle=%u binder=0x%" PRIx64 " cookie=0x%" PRIx64 "\n",
           (uint64_t)off,
           obj->type,
           obj->handle,
           (uint64_t)obj->binder,
...
static int aosp_check_service(int fd, const char *name, uint32_t *out_handle)
{
    ...
    for (;;) {
        ...
        while (ptr + sizeof(uint32_t) <= end) {
            ...
            printf("aosp checkService got %s 0x%08x\n", cmd_name(rcmd), rcmd);
            ...
            if (rcmd == BR_REPLY) {
                ...
                if (!handle) {
                    ...
                    fprintf(stderr, "AOSP checkService returned null handle for %s\n", name);
                    return 1;
                }
                ...
                if (binder_send_handle_cmd(fd,
                                           BC_ACQUIRE,
                                           handle,
                                           "aosp checkService BC_ACQUIRE returned handle") != 0) {
                    if (reply.data.ptr.buffer)
                        binder_free_buffer(fd, reply.data.ptr.buffer);
                    return -1;
                }
            ...
            if (rcmd == BR_DEAD_REPLY || rcmd == BR_FAILED_REPLY) {
                fprintf(stderr, "aosp checkService failed cmd=0x%08x\n", rcmd);
                return 1;
            }
            ...
            if (rcmd == BR_INCREFS ||
                ...
                rcmd == BR_RELEASE ||
                rcmd == BR_DECREFS) {
                if (handle_ref_cmd(fd, rcmd, &ptr, end, "aosp checkService") != 0)
                    return -1;
                continue;
            }

            fprintf(stderr, "aosp checkService unhandled cmd=0x%08x\n", rcmd);
            return -1;
        }
    }
}
        ...
        while (ptr + sizeof(uint32_t) <= end) {
            ...
            if (rcmd == BR_REPLY) {
                ...
                if (reply.data.ptr.buffer &&
                    ...
                    printf("aosp echo reply status=%u text=%s\n",
                           txt->status,
                           txt->text);
            ...
            if (rcmd == BR_DEAD_REPLY || rcmd == BR_FAILED_REPLY) {
                fprintf(stderr, "aosp echo failed cmd=0x%08x\n", rcmd);
                return 1;
            }
            ...
            if (rcmd == BR_INCREFS ||
                ...
                rcmd == BR_RELEASE ||
                rcmd == BR_DECREFS) {
                if (handle_ref_cmd(fd, rcmd, &ptr, end, "aosp echo") != 0)
                    return -1;
                continue;
            }

            fprintf(stderr, "aosp echo unhandled cmd=0x%08x\n", rcmd);
            return -1;
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    printf("AOSP checkService got handle=%u\n", handle);
